...
async def get_upcoming_birthdays(db: AsyncSession):
    ...
    stmt = select(Contact).filter(
        func.DATE_FORMAT(cast(Contact.birthdate, String), "%d.%m") >= func.DATE_FORMAT(today, "%d.%m"), func.DATE_FORMAT(cast(Contact.birthdate, String), "%d.%m") <= func.DATE_FORMAT(next_week, "%d.%m"))
    contacts = await db.execute(stmt)
    upcoming_birthdays = []
